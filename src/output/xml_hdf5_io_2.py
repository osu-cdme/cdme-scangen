...
class XMLWriter():
    ...
    polygon delay 
    '''
    def make_velocity_profiles(self,model:Model):
        ...
        for style in buildStyleList:
            ...
            SubElement(vProfile,'ID').text=str(i)
            i+=1
            SubElement(vProfile,'Velocity').text=str(style.laserSpeed)
            SubElement(vProfile,'Mode').text=str(style.laserMode)
            SubElement(vProfile,'LaserOnDelay').text="500" ##<---- TODO: locate this information in data generated by pyslm
            SubElement(vProfile,'LaserOffDelay').text="0"##<---- TODO: locate this information in data generated by pyslm
            SubElement(vProfile,'JumpDelay').text=str(style.jumpDelay)
            SubElement(vProfile,'MarkDelay').text=str(style.jumpSpeed)
            SubElement(vProfile,'PolygonDelay').text="0"##<---- TODO: locate this information in data generated by pyslm
    ...
    spotsize
    '''
    def make_segment_styles(self,model: Model):
        ...
        for style in buildStyleList:
            ...
            SubElement(traveler, 'ID').text = "Independent"
            SubElement(traveler, 'SyncDelay').text =str(style.jumpDelay)
            SubElement(traveler, 'Power').text =str(style.laserPower)
            SubElement(traveler, 'SpotSize').text =str(style.laserFocus)
    ...
    def make_traj_list(self,layer: Layer):
        ...
        for group in layer.getContourGeometry():
            ...
            SubElement(Start,"Y").text=str(startPair[-1]) ## <- likely error point, should reference y coordinate but threw error when looking at index 1
    ...
    def output_xml(self, layers: List[Layer], model:Model):
        ...
        for i in range(0, len(layers)):
            ...
            print(xml_path)
            self.write_layer(model,layers[i],i+1)
